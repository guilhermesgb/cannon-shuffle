...
import cannon.shuffle.bullet.LightningBullet;
...
public class LightningBarrel extends Barrel{
	...
	long onTouchTime;
	long touchHoldTime;
	long maxTouchTime=(long) ((float)Constants.MAX_BULLET_SPEED/(Constants.BULLET_SPEED*constantTime))/3;
	public BarrelPart barrel_part;
	public ShieldPart shield_part;

	public class LightningBarrelPart extends BarrelPart{

		public LightningBarrelPart(BodyType bodyType, Vector2 pos, float angle,
				World world) {
			super(bodyType, pos, angle, world);
			specificType = Entities.LIGHTNING_BARREL_PART;
			protection = -0.2f;
		}
	}
	public class LightningShieldPart extends ShieldPart{

		public LightningShieldPart(BodyType bodyType, Vector2 pos, float angle,
				World world) {
			super(bodyType, pos, angle, world);
			specificType = Entities.LIGHTNING_SHIELD_PART;
			protection =0.5f;
		}
	}

	public LightningBarrel(World world, Vector2 pos){
		...
		barrel_part = new LightningBarrelPart(null, null, -1, null);
		shield_part = new LightningShieldPart(null, null, -1, null);
	...
	public void update(World world, SpriteBatch batch, Cannon cannon, Array<Bullet> bullets, OrthographicCamera camera){
		...
		if ( Gdx.input.isTouched() && !Utils.isTouchingCannon() && !GameScreen.cannon.blockShoot ){
			...
			if(touchHoldTime>50 && chargeRatio != 1){
				shoot();
			}
		...
		if(da>resolution){
			...
			if(has_shot){
				...
				LightningBullet bullet = new LightningBullet(world, bullet_pos, angle, false);
				bullets.add(bullet);
...
//				float speed = 2*Constants.BULLET_SPEED*power > Constants.MAX_BULLET_SPEED*2 ? Constants.MAX_BULLET_SPEED*2 : 2*Constants.BULLET_SPEED*power;
				float speed = 8;
	...
	public void setTransform(Vector2 vector2, float angle2) {
		barrel_body.setTransform(vector2, angle2);
		shield_body.setTransform(vector2, angle2);
	}
	...
	public String getSpecificType() {
		System.out.println(barrel_part.specificType);
